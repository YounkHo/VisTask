function drawRadar(viewId, item_id){
	...
	d3.csv("../data/item.csv", function(error, dataset){
		...
		for(var i=0;i<dataset.length;i++){
		    ...
		    Name: dataset[i]['name'],
		    axes: [
	//	      {axis: "总分 (star)", value: dataset[i]['star']}, 
	//	      {axis: "味道 (taste)", value: dataset[i]['taste']}, 
	//	      {axis: "环境 (enviroment)", value: dataset[i]['environment']},  
	//	      {axis: "服务 (service)", value: dataset[i]['service']},  
	//	      {axis: "消费 (cost)", value: dataset[i]['cost']}
